...
| `geometric` | No keypoint model. It reads the person mask, splits it into per-leg components, and uses PCA to get the heel→toe axis. The toe is the end away from the bottom edge, and the heel is where the width narrows behind the ball of the foot | — |

`geometric` reads masks from `data/masks/<stem>.png`, which the Apple Vision CLI (segment 2) produces. Without masks it reports every foot as missed.
...
uv run python -m spike.run --candidates rtmw-full,geometric
...
- **GO** — a keypoint candidate reaches PCK@0.05 ≥ 0.8 in the `top` view, **or** `geometric` reaches an axis error ≤ 8° with a toe error ≤ 5% of foot length.
